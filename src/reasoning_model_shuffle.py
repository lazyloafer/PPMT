...
class ReasoningModel(BertPreTrainedModel):
    ...
    def encode_ent_rela(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None,
        ...
        output = self.bert(input_ids=input_ids,
                           ...
                           use_memory=True).last_hidden_state
    ...
    def encode_query(self, inputs_embeds, attention_mask=None, token_type_ids=None, position_ids=None,
                     add_special_tokens_ids=None, is_add_special_tokens=None):
    ...
    def training_loss(self, batch, lamd):
        # print()
        ...
        seq_embeddings = self.encode_query(inputs_embeds=input_node_embeddings, attention_mask=batch['adj'])[0]
        ...
        cls_loss_fn_2 = nn.CrossEntropyLoss()
        ...
        return loss  # lamd * classification_loss + 1 * loss
    ...
    def predict_score(self, batch, candidate_rep, schema):
        ...
        if batch["type"][0][0] in [0, 1, 2, 3, 4, 5, 6]:
            ...
            input_node_embeddings1 = torch.stack(torch.chunk(ent_rel_embeddings1, chunks=batch['adj1'].size(0), dim=0),
                                                 dim=0)
            seq_embeddings1 = self.encode_query(inputs_embeds=input_node_embeddings1, attention_mask=batch['adj1'])[0]
            ...
            input_node_embeddings2 = torch.stack(torch.chunk(ent_rel_embeddings2, chunks=batch['adj2'].size(0), dim=0),
                                                 dim=0)
            seq_embeddings2 = self.encode_query(inputs_embeds=input_node_embeddings2, attention_mask=batch['adj2'])[0]
    ...
    def forward(self, batch, is_prepare=False, lamd=0.3, candidate_rep=None, schema='matching'):
        ...
        if self.training:
            return self.training_loss(batch, lamd)
        else:
