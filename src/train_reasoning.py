...
from reasoning_dataset_shuffle import ReasoningDataset, qa_collate
from reasoning_dataset import ReasoningChainDataset, ReasoningInterDataset, ReasoningInterChainDataset
from reasoning_model_shuffle import ReasoningModel
        ...
        if not args.init_checkpoint:
            ...
            for epoch in range(start_epoch, int(args.num_train_epochs)):
                ...
                for batch in tqdm(train_dataloader):
                    ...
                    loss = model(batch, is_prepare=False, schema=args.training_inference_schema)
                    if n_gpu > 1:
                        loss = loss.mean()
...
def predict(args, entity_to_feed, model, eval_dataloader, logger, epoch, is_test=False):
    ...
    for i, each_eval_dataloader in enumerate(eval_dataloader):
        ...
        print('Saving {} predicted scores at Epoch {}'.format(qtype[i], str(epoch)))
        np.save('pred_scores/{}.npy'.format(qtype[i]), all_logits)
